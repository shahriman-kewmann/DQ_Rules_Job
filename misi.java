...
String dependency_intra_op = "and";

String main_query = "(main_column1 = '') or (main_column2 = '')";
...
if (mainColumn.contains(",")) {
	...
	tempMainQuery = main_query;
	for (int i=0; i < mainColumns.length; i++) { 
		tempMainQuery = tempMainQuery.replace("main_column"+Integer.toString(i+1), mainColumns[i]);
	}
} else {
	String[] temp_columns = mainColumn.split("\\.");
	temp_columns[temp_columns.length-1] = "`"+temp_columns[temp_columns.length-1]+"`";
...
if (subTable.length() > 0) {
	if (subColumn.contains(",")) {
		...
		for (int i=0; i < subColumns.length; i++) {
			...
			tempSubQuery = tempSubQuery.replace("sub_column"+Integer.toString(i+1), subColumns[i]);
...
if (dependencyTable.length() > 0) {
	if (dependencyColumn.contains(",")) {
		...
		for (int i=0; i < dependencyColumns.length; i++) {
			...
			tempDependencyQuery = tempDependencyQuery.replace("dependency_column"+Integer.toString(i+1), dependencyColumns[i]);
			tempQuery[i] = tempQuery[i].replace("dependency_table", dependencyColumns[i].split("\\.")[0]);
		}
		tempDependencyQuery = String.join(" "+dependency_intra_op+" ", tempQuery); 
	...
	if (mainColumn.contains(",")) {
		...
		for (int i=0; i < mainColumns.length; i++) {
			...
			temp_column = String.join(".", temp_columns);
			tempColumns[i] = temp_column;
			tempDependencyQuery = tempDependencyQuery.replace("main_column"+Integer.toString(i+1), tempColumns[i]);
		}
	} else {
		tempDependencyQuery = tempDependencyQuery.replace("main_column1", mainColumn);
	}
}
